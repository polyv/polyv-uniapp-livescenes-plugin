...
- (void)handleMarquee:(PLVChannelInfoModel *)channel customNick:(NSString *)customNick completion:(void (^)(PLVMarqueeModel * model, NSError *error))completion {
    switch (channel.marqueeType) {
        ...
        case PLVChannelMarqueeType_URL: {
            if (channel.marquee) {
                //设置有自定义跑马灯Code
                NSString *marqueeCode = [PLVLiveSDKConfig sharedSDK].marqueeCode;
                if (!PLV_SafeStringForValue(marqueeCode)) {
                    marqueeCode = @"";
                }
                [PLVLiveVideoAPI loadCustomMarquee:[NSURL URLWithString:channel.marquee] withChannelId:channel.channelId.integerValue userId:channel.accountUserId code:marqueeCode completion:^(BOOL valid, NSDictionary *marqueeDict) {
                    if (valid) {
